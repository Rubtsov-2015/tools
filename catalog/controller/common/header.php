...
class ControllerCommonHeader extends Controller {
	public function index() {
		...
		$data['text_all2'] = $this->language->get('text_all2');
		...
		foreach ($categories as $category) {
			if ($category['top']) {
				...
				foreach ($children as $child) {
					...
					$childs = $this->model_catalog_category->getCategories($child['category_id']);

					foreach ($childs as $ch) {
						$filter_data2 = array(
							'filter_category_id'  => $ch['category_id'],
							'filter_sub_category' => true
						);
						$category_info = $this->model_catalog_category->getCategory($ch['category_id']);
						if ($category_info['image']) {
							$thumb = $this->model_tool_image->resize($category_info['image'], 68,45);
						...
						$chil_data[] = array(
							'name'  => $ch['name'] . ($this->config->get('config_product_count') ? ' (' . $this->model_catalog_product->getTotalProducts($filter_data2) . ')' : ''),
							'image'=>$thumb,
							'href'  => $this->url->link('product/category', 'path=' . $category['category_id'] . '_' . $child['category_id']. '_' . $ch['category_id'])
						);
					}
